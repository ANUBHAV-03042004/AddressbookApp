...
@org.springframework.boot.webmvc.test.autoconfigure.WebMvcTest(AddressbookController.class)
...
class AddressbookControllerTest {
    ...
    // ── DELETE /deletecontact/{firstName}/{lastName} ──────────────────────────

    @Test
    @DisplayName("DELETE /deletecontact: returns 200 and message when contact deleted")
    void deleteContact_returns200WhenDeleted() throws Exception {
        when(addressBookService.deleteContact("John", "Doe")).thenReturn(true);

        mockMvc.perform(delete("/deletecontact/John/Doe"))
               .andExpect(status().isOk())
               .andExpect(content().string("Contact deleted."));
    }

    @Test
    @DisplayName("DELETE /deletecontact: returns 404 when contact not found")
    void deleteContact_returns404WhenNotFound() throws Exception {
        when(addressBookService.deleteContact("Ghost", "User")).thenReturn(false);

        mockMvc.perform(delete("/deletecontact/Ghost/User"))
               .andExpect(status().isNotFound())
               .andExpect(content().string("Contact not found"));
    }

    @Test
    @DisplayName("DELETE /deletecontact: path variables are passed to service")
    void deleteContact_pathVariablesPassedToService() throws Exception {
        when(addressBookService.deleteContact("Alice", "Smith")).thenReturn(true);

        mockMvc.perform(delete("/deletecontact/Alice/Smith"));

        verify(addressBookService).deleteContact("Alice", "Smith");
    }
}
